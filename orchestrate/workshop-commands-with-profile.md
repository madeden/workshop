...
kubectl cluster-info # Default Credentials: admin/admin
...
aws ec2 --profile canonical --region us-west-2 describe-instances \
  --filters "Name=tag:juju-units-deployed,Values=*kubernetes-master*" | \
  jq --raw-output '.[][].Instances[].InstanceId' | \
  xargs -I {} aws ec2 --profile canonical --region us-west-2 associate-iam-instance-profile \
    --iam-instance-profile Name=k8sMaster-Instance-Profile \
    --instance-id {}

aws ec2 --profile canonical --region us-west-2 describe-instances --filters "Name=tag:juju-units-deployed,Values=kubernetes-worker*" | \
  jq --raw-output '.[][].Instances[].InstanceId' | \
  xargs -I {} aws ec2 --profile canonical --region us-west-2 associate-iam-instance-profile --iam-instance-profile Name=k8sWorker-Instance-Profile --instance-id {}
...
for KEY in KubernetesCluster juju-controller-uuid juju-model-uuid; do
  aws ec2 --profile canonical --region us-west-2 describe-instances | \
    ...
    sort | uniq | \
    xargs -I {} aws ec2 --profile canonical --region us-west-2 delete-tags --resources {}  --tags "Key="${KEY}
done

aws ec2 --profile canonical --region us-west-2 describe-instances | \
  jq --raw-output '.[][].Instances[] | select( .Tags[].Value  | contains ("k8s")) | .SecurityGroups[0].GroupId' | \
  sort | uniq | \
  xargs -I {} aws ec2 --profile canonical --region us-west-2 delete-tags --resources {} --tags "Key=juju-model-uuid"

aws ec2 --profile canonical --region us-west-2 describe-instances | \
  jq --raw-output '.[][].Instances[] | select( .Tags[].Value  | contains ("k8s")) | .SecurityGroups[0].GroupId' | \
  sort | uniq | \
  xargs -I {} aws ec2 --profile canonical --region us-west-2 delete-tags --resources {} --tags "Key=juju-controller-uuid"

juju status kubernetes-worker --format json | \
  jq -r '.machines[]."instance-id"' | \
  xargs -I {} aws ec2 --profile canonical --region us-west-2 describe-instances --instance-ids {} | \
  jq --raw-output '.[][].Instances[].SubnetId' | sort | uniq | \
  xargs -I {} aws ec2 --profile canonical --region us-west-2 create-tags --resources {} --tags "Key=KubernetesCluster,Value=workshop"
...
kubectl scale deploy wordpress-ingress-wordpress --replicas=3
...
##### Get Workshop Name #####
juju status --format json | jq -r '.machines[]."instance-id"' | head -n1 | xargs -I {} aws ec2 --profile canonical --region us-west-2 describe-instances --instance-ids {} | jq '.[][].Instances[].Tags'
# {
...
VPC_ID=$(aws ec2 --profile canonical --region us-west-2 describe-vpcs | jq -r '.[][].VpcId')
echo $VPC_ID

SUBNET_IDS=$(aws ec2 --profile canonical --region us-west-2 describe-subnets | jq -r '.[][].SubnetId')
echo $SUBNET_IDS

SG_ID="$(aws ec2 --profile canonical --region us-west-2 describe-instances \
